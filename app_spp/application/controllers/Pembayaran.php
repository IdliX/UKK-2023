...
class Pembayaran extends CI_Controller {
	...
	public function index(){
		...
		if($cek_user_level == 'admin' or $cek_user_level == 'petugas'){
			...
			$this->load->view('tampilandatapembayaran/v_pembayaran',$data);
		}
		else{
			redirect('notfound');
		}
			...
			if($this->input->post('proses')){
				...
				if ($this->form_validation->run() == TRUE){
					...
					if($cekbayar['result'] == "success"){
						...
						$tgl_sekarang = date("Ymd");
						$this->load->view('tampilandatapembayaran/v_pembayaranform',$data);
					}
				}
				else{
					//masukkan data POST ke dalam variabel $array
	...
	function kwitansi($slug){ //fungsi untuk menampilkan detail kwitansi
		...
		if($cek_user_level == 'admin' or $cek_user_level == 'petugas'){
			...
			if($sql->num_rows() > 0){
				...
				$this->load->view('tampilandatapembayaran/v_pembayarankwitansi',$data);
			}
			else{
